 ...
 if(pid_main==0){
   ...
   if(pid1 ==0){
       ...
       if(pid1a ==0){
         ...
         count++;
         printf("Child 1a, adr = %p, count = %d, pid = %d\n",&count,count,getpid());
         printf("Parent process = %d, pid child 1a %d\n",getpid(), pid1a);
      }
   }else if(pid1 > 0){
    count++;
    printf("Child 1, adr = %p, count = %d, pid = %d\n",&count,count,getpid());
    printf("Parent process pid = %d, pid child 1 %d\n",getpid(), pid1);
   }
 }else if(pid_main>0){
    count++;
    printf("Child main, adr = %p, count = %d, pid = %d\n",&count,count,getpid());
    printf("Parent process pid = %d, pid child main %d\n",getpid(), pid_main);
    pid2 = fork();
    if(pid2==0){
       count++;
       printf("Child process\n");
       ...
       if(pid2a == 0){
         ...
         count++;
         printf("Child 2a, adr = %p, count = %d, pid = %d\n",&count,count,getpid());
         printf("Parent process pid = %d, pid child 2a %d\n",getpid(), pid2a);
       }
    }else if(pid2>0){
       count++;
       printf("Child 2, adr = %p, count = %d, pid = %d\n",&count,count,getpid());
       printf("Parent process pid = %d, pid child 2 %d\n",getpid(), pid2);
    }
 }
 sleep(10);
 return 0;
